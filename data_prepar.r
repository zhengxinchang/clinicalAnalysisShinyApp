...
data_prepare <- function(){
  ...
  unified_data$induction1_outcome <- factor(unified_data$induction1_outcome,levels = c("CR","CRi","PR","NR"))
  unified_data$induction2_outcome <- factor(unified_data$induction2_outcome,levels=c("CR","CRi","PR","NR"))
  ...
  # 2020年12月23日 修复OS数值
  # 开始的时间以初诊日期开始计算，这里标记清楚
  # 患者的induction1的时间为start time ，如果发生死亡事件，则dead_date --> OS，
  # 如果发生了放弃，则添加放弃日期 abandon_date -->OS, 
  # 之后将所有留空的事件填充followup_date
  
  unified_data$OS <- NA
  unified_data$Status_OS <-NA
  for(i in seq(1,nrow(unified_data))){
    if (isTRUE(unified_data$is_dead[i])){
      unified_data$OS[i]<- (as.numeric(unified_data$dead_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_OS[i]<- 1
    }else if (isTRUE(unified_data$is_abandon[i])){
      unified_data$OS[i]<- (as.numeric(unified_data$abandon_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_OS[i]<- 0
    }else if(isFALSE(unified_data$is_dead[i])) {
      unified_data$OS[i]<- (as.numeric(unified_data$follow_up_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_OS[i]<- 0
    } 
  ...
  # 5.计算EFS1
  # 5.1 患者的induction1的时间为start time ，如果发生死亡,复发，诱导缓解失败事件，
  # 则death_date --> EFS1 ,relapse_date -->EFS1  NR -EFS1=0，
  # 如果发生了放弃，则abandon_date -->EFS1,  发生死亡，复发，NR，
  # 放弃的患者EFS_Status =1 其他为0  之后将所有留空的事件填充followup_date
  ...
  # 
  unified_data$EFS<-NA
  unified_data$Status_EFS<-NA
  for(i in seq(1,nrow(unified_data))){
    if(isTRUE(unified_data$is_dead[i])){
      unified_data$EFS[i]  <- (as.numeric(unified_data$dead_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS[i] <- 1
    }else if (isTRUE(unified_data$is_relapse[i])){
      unified_data$EFS[i]  <- (as.numeric(unified_data$relapse_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS[i] <- 1
    }else if (isTRUE(unified_data$induction1_outcome[i] == "NR")){
      unified_data$EFS[i]<-0
      unified_data$Status_EFS[i]<-1
    }else if(isTRUE(unified_data$is_abandon[i])){
      unified_data$EFS[i]<-(as.numeric(unified_data$abandon_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS[i]<-1
    }else {
      unified_data$EFS[i]<-(as.numeric(unified_data$follow_up_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS[i]<-0
      
    }
    
  }
  
  #        6.1 患者的induction1的时间为start time ，如果发生死亡,复发，诱导缓解
  #        失败事件，则death_date --> EFS2 ,relapse_date -->EFS1  NR -EFS2=0，如
  #        果发生了放弃，则abandon_date -->EFS2,  发生死亡，复发，NR，患者
  #        EFS_Status =1 其他为0  之后将所有留空的事件填充followup_date
  #        
  unified_data$EFS2<-NA
  unified_data$Status_EFS2<-NA
  for(i in seq(1,nrow(unified_data))){
    if(isTRUE(unified_data$is_dead[i])){
      unified_data$EFS2[i]  <- (as.numeric(unified_data$dead_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS2[i] <- 1
    }else if (isTRUE(unified_data$is_relapse[i])){
      unified_data$EFS2[i]  <- (as.numeric(unified_data$relapse_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS2[i] <- 1
    }else if (isTRUE(unified_data$induction1_outcome[i] == "NR")){
      unified_data$EFS2[i]<-0
      unified_data$Status_EFS2[i]<-1
    }else if(isTRUE(unified_data$is_abandon[i])){
      unified_data$EFS2[i]<-(as.numeric(unified_data$abandon_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS2[i]<-0
    }else {
      unified_data$EFS2[i]<-(as.numeric(unified_data$follow_up_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_EFS2[i]<-0
  ...
  # 患者的induction1的时间为start time ，如果发生复发，则relapse_date --> RFS, 
  #  如果NR则RFS= 0 ，如果死亡，则 death_date --> RFS , 且 RFS_status = 2 
  unified_data$RFS<-NA
  unified_data$Status_RFS<-NA
  for(i in seq(1,nrow(unified_data))){
    if (isTRUE(unified_data$is_relapse[i])){
      unified_data$RFS[i] <- (as.numeric(unified_data$relapse_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_RFS[i] <- 1
    }else if (isTRUE(unified_data$is_dead[i])){
      unified_data$RFS[i] <- (as.numeric(unified_data$dead_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_RFS[i] <- 2
    }else if(isTRUE(unified_data$induction1_outcome[i] == "NR")){
      unified_data$RFS[i] <-0
      unified_data$Status_RFS[i]<-1      
    }else {
      unified_data$RFS[i] <-(as.numeric(unified_data$follow_up_date[i]) - as.numeric(unified_data$diagnosis_date[i]))/30
      unified_data$Status_RFS[i]<-0 
    }
  ...
  gene_mutations<- unified_data %>% select(CKIT,NRAS,NPM1,CEBPA,ASXL1,induction1_outcome,induction2_outcome,Treatment,patient_id) %>%
    gather(key="Mutation",value = "status",-c(induction1_outcome,induction2_outcome,Treatment,patient_id)) %>% na.omit()
